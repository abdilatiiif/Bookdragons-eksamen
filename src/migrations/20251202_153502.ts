...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`books_genre\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`books_genre_order_idx\` ON \`books_genre\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`books_genre_parent_idx\` ON \`books_genre\` (\`parent_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`books_genre\`;`)
